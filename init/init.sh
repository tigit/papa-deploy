...
function install_base
{
    ...
    apt-get install -y libreadline-dev libncurses5-dev libpcre3-dev libssl-dev perl make build-essential git
...
function install_openresty
{
	...
	ln -s -f luajit-2.1.0-alpha luajit && \
	ln -s -f luajit-2.1.0-alpha lua
    ...
    ## 创建目录
	mkdir -p ${resty_var}/log && mkdir -p ${resty_var}/run && chown www:www -R ${resty_var}
